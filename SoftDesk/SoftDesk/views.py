...
class ProjectsAPIView(APIView):
    ...
    def get(self, request, id=None):
        if Contributors.objects.filter(user=request.user).exists() is not True:
            if id is not None:
                queryset = Projects.objects.filter(project_id=id, author_user=request.user)
                if queryset:
    ...
    def put(self, request, id):
        if Contributors.objects.filter(user=request.user).exists() is not True:
            if Projects.objects.filter(project_id=id, author_user=request.user).exists():
                objects = Projects.objects.get(project_id=id, author_user=request.user)
                context = {'request': request}
    ...
    def delete(self, request, id):
        if Contributors.objects.filter(user=request.user).exists() is not True:
            if Projects.objects.filter(project_id=id, author_user=request.user).exists():
                Projects.objects.get(project_id=id).delete()
                all_issues = Issues.objects.filter(project=id).all()
...
class ContributorsAPIView(APIView):
    ...
    def post(self, request, id):
        if Contributors.objects.filter(user=request.user).exists() is not True:
            ...
            return Response({"You dont have the permission to add a contributor to a project"}, status=status.HTTP_400_BAD_REQUEST)

    def get(self, request, id):
        if Contributors.objects.filter(user=request.user).exists() is not True:
            objects = ContributorSerializer(Contributors.objects.filter(project=id), many=True)
            if not objects.data:
                return Response("There are no contributer(s) linked to this project")
            else:
                return Response(objects.data, status=status.HTTP_200_OK)
        else:
            return Response({"You dont have the permission to view the contributor(s) linked to a project"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def delete(self, request, project_id, contributor_id):
        if Contributors.objects.filter(user=request.user).exists() is not True:
            if Projects.objects.filter(author_user=request.user, project_id=project_id).exists():
                if Projects.objects.filter(project_id=project_id).exists() and Contributors.objects.filter(user=contributor_id).exists():
                    ...
                    return Response({"The id of either the project or the contributor was not found"}, status=status.HTTP_404_NOT_FOUND)
            else:
            ...
            return Response({"You dont have the permission to delete a contributor from a project"}, status=status.HTTP_400_BAD_REQUEST)
class IssuesAPIView(APIView):
    ...
    def post(self, request, id):
        if Contributors.objects.filter(user=request.user).exists():
            ...
            return Response({"You dont have the permission to add an issue to a project"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, project_id, issue_id):
        if Contributors.objects.filter(user=request.user).exists():
            if Issues.objects.filter(pk=issue_id, assignee_user=request.user).exists():
                objects = Issues.objects.get(pk=issue_id, assignee_user=request.user)
            ...
            return Response({"You dont have the permission to update an issue to a project"}, status=status.HTTP_400_BAD_REQUEST)

    def delete(self, request, project_id, issue_id):
        if Contributors.objects.filter(user=request.user).exists():
            if Projects.objects.filter(project_id=project_id).exists() and Issues.objects.filter(pk=issue_id).exists():
                if Issues.objects.filter(project=project_id, pk=issue_id, assignee_user=request.user).exists():
                    issue = Issues.objects.get(project=project_id, pk=issue_id, assignee_user=request.user)
                    all_comments = Comments.objects.filter(issue=issue.id).all()
                    issue.delete()
                    if all_comments is not None:
                        for comment in all_comments:
                ...
                return Response({"The id of either the project or the issue was not found"}, status=status.HTTP_404_NOT_FOUND)
        else:
            return Response({"You dont have the permission to delete an issue to a project"}, status=status.HTTP_400_BAD_REQUEST)

class CommentsAPIView(APIView):
    permission_classes = (permissions.IsAuthenticated,)
    serializer_class = CommentSerializer

    def get(self, request, project_id, issue_id, id=None):
        if id is not None:
            queryset = Comments.objects.filter(comment_id=id)
            if queryset:
    ...
    def post(self, request, project_id, issue_id):
        if Contributors.objects.filter(user=request.user).exists():
            ...
            return Response({"You dont have the permission to create a comment to an issue"}, status=status.HTTP_400_BAD_REQUEST)

    def put(self, request, project_id, issue_id, id):
        if Contributors.objects.filter(user=request.user).exists():
            if Comments.objects.filter(comment_id=id, author_user=request.user).exists():
                objects = Comments.objects.get(comment_id=id, author_user=request.user)
                context = {'request': request, 'project_id': project_id, 'issue_id': issue_id}
            ...
            return Response({"You dont have the permission to update a comment to an issue"}, status=status.HTTP_400_BAD_REQUEST)


    def delete(self, request, project_id, issue_id, id):
        if Contributors.objects.filter(user=request.user).exists():
            if Projects.objects.filter(project_id=project_id).exists():
                if Issues.objects.filter(pk=issue_id).exists() and Comments.objects.filter(comment_id=id).exists():
                    if Comments.objects.filter(comment_id=id, issue=issue_id, author_user=request.user).exists():
                        Comments.objects.get(comment_id=id, issue=issue_id, author_user=request.user).delete()
                        return Response({
            ...
            return Response({"You dont have the permission to delete a comment to an issue"}, status=status.HTTP_400_BAD_REQUEST)